...
class GeneratedDocsTests(GeneratedCodeTestCase):
    ...
    def test_generate_code_writes_rustdoc_for_solver_api(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("/// Result returned by [`CGRProblem::solve`] and [`CGRProblem::solve_with_settings`].", lib_text)
            self.assertIn("pub struct SolveResult", lib_text)
            self.assertIn("/// Errors returned by the generated Rust solver API.", lib_text)
    ...
    def test_generate_code_writes_readme_documentation(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertNotIn("Rust setters for Clarabel options", readme_text)
            self.assertNotIn("Element-wise parameter update functions", readme_text)
