...
class MetadataTests(GeneratedCodeTestCase):
    ...
    def test_generate_code_writes_crate(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("SUPPORTED_SOLVER_SETTINGS", wrapper_text)
            self.assertIn("'max_iter'", wrapper_text)
            self.assertIn("'tol_feas'", wrapper_text)
            self.assertIn("'pardiso_iparm'", wrapper_text)
